...
import { updatedevice } from '../actions/phone';
import { setAlert } from '../actions/alert';

const Update = ({ updated, phones, updatedevice, setAlert, failed, usertype }) => {
    const lis = [];
    const [namee, changename] = useState('Select Device');
    const [valone, changevalone] = useState('');
    ...
    phones.ress.forEach((pro) => {
        ...
        if (usertype === 'Admin') {
            if (pro.name.includes(valone) && valone !== "") {
                lis.push(<Dropdown.Item onClick={() => { changename(pro.name); setformdata(pro); changevalone(''); }}><h6 style={{ margin: '0' }}>{pro.name}</h6></Dropdown.Item>);
            }
        }
        else {
            if (pro.name.includes(valone) && valone !== "" && pro.user === localStorage.getItem('id')) {
                lis.push(<Dropdown.Item onClick={() => { changename(pro.name); setformdata(pro); changevalone(''); }}><h6 style={{ margin: '0' }}>{pro.name}</h6></Dropdown.Item>);
            }
        }
    });
    ...
    return (
        <div>
            <p style={{ textAlign: "center" }}>Note : You can Update only those devices that you created</p>
            <Dropdown>
                <Dropdown.Toggle variant="secondary" id="dropdown-basic" className="reportsearch">
                    {namee}
                </Dropdown.Toggle>
                <Dropdown.Menu className="reportresult">
                    <input type="text" style={{ width: "97%", height: "40%", marginLeft: "2%" }} onChange={showit} placeholder="Search a Device..." value={valone}></input>
                    {lis}
                </Dropdown.Menu>
            </Dropdown>
            <div style={{ padding: "4%", width: "60%", margin: "0 auto", borderRadius: "10px" }}>
                <Alert />
                ...
                <form onSubmit={justSubmit}>
                    ...
                    <br /><br />
                    <input type="submit" className="btn btn-danger btn-lg" value="UPDATE" />
                </form>
            </div>
        </div>
    );
...
export default connect(mapStateToProps, { updatedevice, setAlert })(Update);